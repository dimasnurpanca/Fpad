...
public class ReadingListsAdapter extends RecyclerView.Adapter<ReadingListsViewHolder> {

    private ArrayList<StoryList> dataSet;
    Context mContext;

    public ReadingListsAdapter(ArrayList<StoryList> data, Context context) {
        this.dataSet=data;
        this.mContext = context;
    }

    @Override
    public ReadingListsViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.reading_list_item, parent, false);
        return new ReadingListsViewHolder(view, mContext, dataSet);
    }

    @Override
    public void onBindViewHolder(ReadingListsViewHolder holder, int position) {
